...
export const volatilityModel: ProjectDemoData = {
  id: "3",
  title: "Volatility Breakout Prediction – Code Demo",
  description:
    "XGBoost model predicting when SPY will move 5x the expanding average true range (measured from market open) within 30 minutes. Achieved 0.686 ROC-AUC on one year of minute-level data.",
  codeSamples: [
    {
      label: "Helper Functions & ATR Calculation",
      description:
        "RSI, ATR, and True Range calculations for volatility measurement",
...
def true_range(df: pd.DataFrame) -> pd.Series:
    ...
    {
      label: "Data Preprocessing & VWAP",
      description:
        "Filter market hours, calculate VWAP, and prepare time features",
...
spy_df = preprocess_dataframe(spy_df)`
    ...
    {
      label: "Multi-Timeframe Volatility Features",
      description:
        "ATR across multiple windows (5, 30, 60, 120 min) and volatility ratios",
...
spy_df = create_volatility_ratio_features(spy_df)`
    ...
    {
      label: "VWAP Distance & Statistical Features",
      description:
        "Price distance from VWAP with rolling statistics for mean reversion signals",
...
spy_df = create_interaction_features(spy_df)`
    ...
    {
      label: "Label Generation - Volatility Breakouts",
      description:
        "Parallel processing to label significant price movements beyond volatility bands",
...
spy_df = label_volatility_breakout_events(
    ...
    {
      label: "XGBoost Model Training",
      description:
        "Train binary classification model with temporal train/test split",
...
print(f"Test AUC: {auc:.4f}")`
    ...
    {
      label: "Model Calibration Analysis",
      description:
        "Visualize prediction calibration and analyze performance by confidence level",
...
plt.show()`
    ...
    {
    label: 'Live Implementation Into Trading Framework',
    description: 'Integrate trained model for real-time volatility breakout predictions',
    code: `"""
